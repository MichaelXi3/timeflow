...
export default function DayPage() {
  ...
  // Load log and set preview mode based on content
  useEffect(() => {
    if (currentLog) {
      setMarkdown(currentLog.markdown);
      // If there's content, show preview mode
      setIsPreview(currentLog.markdown.trim().length > 0);
    } else {
      setMarkdown('');
      // If empty, show edit mode
      setIsPreview(false);
    }
  }, [currentLog]);

  // Auto-generate daily summary at end of day if log is empty
  useEffect(() => {
    // Only run if we have all required data and haven't auto-generated yet
    if (!isInitialized || !slots || !allTags || !allDomains || hasAutoGenerated) return;
    
    // Check if this is a past date (not today or future)
    const now = new Date();
    const today = getStartOfDay(now);
    const currentDayStart = getStartOfDay(currentDate);
    
    // Only auto-generate for past dates
    if (currentDayStart.getTime() >= today.getTime()) return;
    
    // Check if log is empty
    if (currentLog && currentLog.markdown.trim().length > 0) return;
    
    // Filter slots to only include those that start within the current day
    const daySlots = slots.filter((slot) => slot.start >= dayStart && slot.start < dayEnd);
    
    // Only generate if there are slots for this day
    if (daySlots.length === 0) return;
    
    // Auto-generate the summary
    const domainStats = calculateDomainStats(daySlots, allTags, allDomains, settings.attributionMode);
    const summary = generateDailySummary(domainStats, dateStr, daySlots, allTags);
    
    // Save the auto-generated summary
    dbHelpers.upsertDailyLog(dateStr, summary).then(() => {
      setHasAutoGenerated(true);
    });
  }, [isInitialized, slots, allTags, allDomains, currentLog, hasAutoGenerated, currentDate, dateStr, dayStart, dayEnd, settings.attributionMode]);
